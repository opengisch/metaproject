...
class PGInheritanceView():
	...
	def sql_merge_update_trigger(self, child):
		if 'merge_view' not in self.definition:
			return ''
		...
		if self.allow_type_change:
			sql += "\n\tIF OLD.{0}_type != NEW.{0}_type THEN".format((self.definition['alias'])
			sql += "\n\t\tCASE"
			for child in self.definition['children']:
				sql += "\n\t\t\tWHEN OLD"
			sql += "\n\t\tEND CASE;"
	...
	def sql_merge_delete_trigger(self, child):
		if 'merge_view' not in self.definition:
			return ''
